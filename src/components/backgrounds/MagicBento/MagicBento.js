...
const ParticleCard = ({ children, className = "", disableAnimations = false, style, particleCount = DEFAULT_PARTICLE_COUNT, glowColor = DEFAULT_GLOW_COLOR, enableTilt = true, clickEffect = false, enableMagnetism = false, }) => {
    ...
    return (_jsx("div", { ref: cardRef, className: `${className} relative overflow-hidden`, style: { ...style, position: "relative", overflow: "hidden" }, children: children }));
};
...
const BentoCardGrid = ({ children, gridRef }) => (_jsx("div", { className: "bento-section grid gap-2 p-3 max-w-7xl select-none relative", style: { fontSize: "clamp(1rem, 0.9rem + 0.5vw, 1.5rem)" }, ref: gridRef, children: children }));
...
const MagicBento = ({ textAutoHide = true, enableStars = true, enableSpotlight = true, enableBorderGlow = true, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, particleCount = DEFAULT_PARTICLE_COUNT, enableTilt = false, glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = true, }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("style", { children: `
        ...
        ` }), enableSpotlight && (_jsx(GlobalSpotlight, { gridRef: gridRef, disableAnimations: shouldDisableAnimations, enabled: enableSpotlight, spotlightRadius: spotlightRadius, glowColor: glowColor })), _jsx(BentoCardGrid, { gridRef: gridRef, children: _jsx("div", { className: "card-responsive grid gap-2", children: cardData.map((card, index) => {
                        ...
                        if (enableStars) {
                            return (_jsxs(ParticleCard, { className: baseClassName, style: cardStyle, disableAnimations: shouldDisableAnimations, particleCount: particleCount, glowColor: glowColor, enableTilt: enableTilt, clickEffect: clickEffect, enableMagnetism: enableMagnetism, children: [_jsx("div", { className: "card__header flex justify-between gap-3 relative text-white", children: _jsx("span", { className: "card__label text-base", children: card.label }) }), _jsxs("div", { className: "card__content flex flex-col relative text-white", children: [_jsx("h3", { className: `card__title font-normal text-base m-0 mb-1 ${textAutoHide ? "text-clamp-1" : ""}`, children: card.title }), _jsx("p", { className: `card__description text-xs leading-5 opacity-90 ${textAutoHide ? "text-clamp-2" : ""}`, children: card.description })] })] }, index));
                        }
                        return (_jsxs("div", { className: baseClassName, style: cardStyle, ref: (el) => {
                                if (!el)
                                    return;
                                const handleMouseMove = (e) => {
