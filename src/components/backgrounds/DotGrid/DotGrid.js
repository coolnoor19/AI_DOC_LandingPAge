...
const DotGrid = ({ dotSize = 16, gap = 32, baseColor = "#6E56FF", activeColor = "#6E56FF", proximity = 140, speedTrigger = 100, shockRadius = 260, shockStrength = 5, maxSpeed = 5000, resistance = 750, returnDuration = 1.5, className = "", style, }) => {
    ...
    const baseRgb = useMemo(() => hexToRgb(baseColor), [baseColor]);
    const activeRgb = useMemo(() => hexToRgb(activeColor), [activeColor]);
    // Prebuild circle path for fast drawing
    const circlePath = useMemo(() => {
    ...
    /** Build/resize grid to fit wrapper */
    const buildGrid = useCallback(() => {
        const wrap = wrapperRef.current;
        const canvas = canvasRef.current;
        if (!wrap || !canvas)
    ...
    return (_jsx("section", { className: `relative h-full w-full ${className}`, style: style, "aria-hidden": "true", children: _jsx("div", { ref: wrapperRef, className: "absolute inset-0", children: _jsx("canvas", { ref: canvasRef, className: "absolute inset-0 w-full h-full pointer-events-none" }) }) }));
};
export default DotGrid;
